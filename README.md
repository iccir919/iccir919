# Hi, I'm Neil 👋

I'm a frontend developer who designs and builds websites that are clean, accessible, and effortless to use. I'm passionate about creating digital experiences that feel natural and intuitive, with a focus on simplicity and user-centered design.

**Location:** East Bay, California  
**Portfolio:** [neilricci.dev](https://www.neilricci.dev/)

## 🛠️ Tech Stack

**Languages & Frameworks:**  
`JavaScript` · `React` · `HTML` · `CSS`

**Tools & Platforms:**  
`Vite` · `Git` · `Vercel` · `OpenAI API` · `ESLint`

**Specialties:**  
AI-powered web applications · Data visualization · API integration · Responsive design

## 🌟 Featured Projects

### 🎢 [Yen Sid's Ride Recommender](https://github.com/iccir919/yen-sid)
AI-powered Disney theme park ride recommendation app that combines real-time data with intelligent suggestions.
- **Tech:** JavaScript, OpenAI API, Theme Parks Wiki API, Weather API
- **Live:** [yen-sid.vercel.app](https://yen-sid.vercel.app)

### 🎬 [Aronofsky Advisor](https://github.com/iccir919/aronofsky-advisor)
React application that recommends films based on Darren Aronofsky's distinctive cinematic style using AI.
- **Tech:** React, Vite, OpenAI API
- **Live:** [aronofsky-advisor.vercel.app](https://aronofsky-advisor.vercel.app)

### ❤️ [Pulse Watch](https://github.com/iccir919/pulseWatch) · 41⭐
Export and visualize your Fitbit heart-rate data with privacy-first, client-side processing.
- **Tech:** HTML, JavaScript, Data Visualization
- **Live:** [pulseWatch Demo](https://iccir919.github.io/pulseWatch/public/index.html)

### 🎵 [See My Sound](https://github.com/iccir919/see-my-sound)
Spotify-powered application that visualizes your music profile and listening data.
- **Tech:** React, Vite, Spotify Web API, OAuth 2.0 PKCE
- **Live:** [see-my-sound.vercel.app](https://see-my-sound.vercel.app)

## 💡 What I'm Working On

- Building AI-powered web applications that solve real-world problems
- Exploring creative uses of LLMs and API integrations
- Creating tools that prioritize user privacy and data ownership
- Learning and experimenting with modern web technologies

## 📫 Connect With Me

[Portfolio](https://www.neilricci.dev/) · [LinkedIn](https://www.linkedin.com/in/neil-ricci/) · [Twitter/X](https://x.com/realNeilVR) · [Email](mailto:neil.ricci9@gmail.com)

---

💭 *"console.log('Hello');"*  
⭐ Open to collaboration and interesting projects!
